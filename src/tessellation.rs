...
pub trait HasTessellation {
  ...
  ///
  /// The `size` argument gives, for points or lines, the size to use as thickness when rendering.
  ///
  /// The `instances` argument gives the number of instances to render.
  fn render(tessellation: &Self::Tessellation, size: Option<f32>, instances: u32);
}
...
impl<C> Tessellation<C> where C: HasTessellation {
  pub fn new<T>(mode: Mode, vertices: &Vec<T>, indices: Option<&Vec<u32>>) -> Tessellation<C> where T: Vertex {
    Tessellation {
