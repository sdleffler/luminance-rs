...
impl GLState {
  ...
  /// > in this case.
  pub(crate) fn new(mut ctx: WebGL2RenderingContext) -> Result<Self, StateQueryError> {
  ...
  fn get_from_context(mut ctx: WebGL2RenderingContext) -> Result<Self, StateQueryError> {
    ...
    let viewport = Self::get_ctx_viewport(&mut ctx)?;
    let clear_color = Self::get_ctx_clear_color(&mut ctx)?;
    let blending_state = Self::get_ctx_blending_state(&mut ctx)?;
    let blending_equation = Self::get_ctx_blending_equation(&mut ctx)?;
    let blending_func = Self::get_ctx_blending_factors(&mut ctx)?;
    let depth_test = Self::get_ctx_depth_test(&mut ctx)?;
    let depth_test_comparison = DepthComparison::Less;
    let face_culling_state = Self::get_ctx_face_culling_state(&mut ctx)?;
    let face_culling_order = Self::get_ctx_face_culling_order(&mut ctx)?;
    let face_culling_mode = Self::get_ctx_face_culling_mode(&mut ctx)?;
    let vertex_restart = Self::get_ctx_vertex_restart(&mut ctx)?;
    let patch_vertex_nb = 0;
    let current_texture_unit = Self::get_ctx_current_texture_unit(&mut ctx)?;
    ...
    let bound_draw_framebuffer = Self::get_ctx_bound_draw_framebuffer(&mut ctx)?;
    let bound_vertex_array = Self::get_ctx_bound_vertex_array(&mut ctx)?;
    let current_program = Self::get_ctx_current_program(&mut ctx)?;
  ...
  fn get_ctx_bound_vertex_array(ctx: &WebGL2RenderingContext) -> Result<GLuint, StateQueryError> {
    let bound = ctx
